...
motor_err_t motor_update_autoreload(motor_t *motor)
{
	...
	motor->autoreload_ =
		(float)__HAL_TIM_GET_AUTORELOAD(motor->config_.timer);
...
motor_err_t motor_stop(motor_t *motor)
{
	...
	HAL_GPIO_WritePin(motor->config_.dir_pin_1_port,
			  motor->config_.dir_pin_1, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(motor->config_.dir_pin_2_port,
			  motor->config_.dir_pin_2, GPIO_PIN_RESET);
...
motor_err_t motor_run(motor_t *motor, float power)
{
	...
	if (power) {
		...
		HAL_GPIO_WritePin(motor->config_.dir_pin_1_port,
				  motor->config_.dir_pin_1,
				  direction ? GPIO_PIN_SET : GPIO_PIN_RESET);
		HAL_GPIO_WritePin(motor->config_.dir_pin_2_port,
				  motor->config_.dir_pin_2,
				  !direction ? GPIO_PIN_SET : GPIO_PIN_RESET);
		__HAL_TIM_SET_COMPARE(motor->config_.timer,
				      motor->config_.channel,
				      fabsf(power) * motor->autoreload_);
			    ...
			    motor_t *br_motor)
{
	...
	MOTOR_CHECK_WITH_RET(motor_get_state(fl_motor, &state),
			     ROBOT_ERR_MOTOR);
	if (!state.initialized)
		return ROBOT_ERR_MOTOR;
	MOTOR_CHECK_WITH_RET(motor_get_state(fr_motor, &state),
			     ROBOT_ERR_MOTOR);
	if (!state.initialized)
		return ROBOT_ERR_MOTOR;
	MOTOR_CHECK_WITH_RET(motor_get_state(bl_motor, &state),
			     ROBOT_ERR_MOTOR);
	if (!state.initialized)
		return ROBOT_ERR_MOTOR;
	MOTOR_CHECK_WITH_RET(motor_get_state(br_motor, &state),
			     ROBOT_ERR_MOTOR);
	if (!state.initialized)
		return ROBOT_ERR_MOTOR;
				    ...
				    float theta, float turn)
{
	...
	if (power == 0.0f && turn == 0.0f)
		return frwd_robot_stop(robot);
