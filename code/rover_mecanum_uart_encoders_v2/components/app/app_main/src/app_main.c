...
#define CJSON_NUMBER_OR_ZERO(x) \
	((cJSON_IsNumber(x) ? cJSON_GetNumberValue(x) : 0.0f))
...
static const osSemaphoreAttr_t robot_sem_attr = { .name = "robot sem" };
...
const osThreadAttr_t app_main_thread_attr = { .name = "app main thread",
					      .stack_size = 4 * 256,
...
void app_main_thread_fun(void *param)
{
	...
	uart_rb_init(&rover_uart_rb, &HUART, rover_uart_rb_rx_rb_data,
		     sizeof(rover_uart_rb_rx_rb_data), rover_uart_rb_tx_rb_data,
		     sizeof(rover_uart_rb_tx_rb_data));
	...
	while (1) {
		...
		lwpktr_t res =
			lwpkt_process(&rover_uart_pkt, OS_TICKS_TO_MS(ticks));
		switch (res) {
		case lwpktVALID:
			...
			if (cJSON_IsObject(pkt_json)) {
				...
				cJSON *power_json =
					cJSON_GetObjectItem(pkt_json, "p");
				cJSON *theta_json =
					cJSON_GetObjectItem(pkt_json, "th");
				cJSON *turn_json =
					cJSON_GetObjectItem(pkt_json, "tu");
				cJSON *stop_json =
					cJSON_GetObjectItem(pkt_json, "stop");

				if (cJSON_IsTrue(stop_json)) {
					osSemaphoreAcquire(robot_sem_id,
							   osWaitForever);
					frwd_robot_stop(&robot);
					osSemaphoreRelease(robot_sem_id);
					printf("stop\r\n");
				} else if (cJSON_IsNumber(power_json) ||
					   cJSON_IsNumber(theta_json) ||
					   cJSON_IsNumber(turn_json)) {
					float power = CJSON_NUMBER_OR_ZERO(
						power_json);
					float turn =
						CJSON_NUMBER_OR_ZERO(turn_json);
					float theta = CJSON_NUMBER_OR_ZERO(
						theta_json);

					osSemaphoreAcquire(robot_sem_id,
							   osWaitForever);
					frwd_robot_mecanum_move(&robot, power,
								theta, turn);
					osSemaphoreRelease(robot_sem_id);
					printf("running\r\n");
				} else {
				...
				osTimerStart(
					robot_safety_stop_timer_id,
					OS_MS_TO_TICKS(
						ROBOT_SAFETY_STOP_TIMER_PERIOD_MS));
			}
		...
		if (current_state != last_state) {
			...
			if (current_state == GPIO_PIN_RESET)
				frwd_robot_mecanum_move(&robot, 0.5f, M_PI_2,
							0.0f);
			else
				frwd_robot_stop(&robot);
			osSemaphoreRelease(robot_sem_id);
...
{
	app_main_thread_id =
		osThreadNew(app_main_thread_fun, NULL, &app_main_thread_attr);
